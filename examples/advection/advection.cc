...
using Problem = hyperdeal::advection::
...
struct ParametersDriver
{
  ...
  add_parameters(dealii::ParameterHandler &prm)
  {
    ...
    prm.add_parameter("DimX", dim_x, "Number of dimension in x-space.");
    prm.add_parameter("DimV", dim_v, "Number of dimension in v-space.");
    prm.add_parameter("DegreeX", degree_x, "Polynomial degree in x-space.");
    prm.add_parameter("DegreeV", degree_v, "Polynomial degree in v-space.");
    prm.add_parameter("PartitionX",
                      partition_x,
                      "Number of partitions of the x-triangulation.");
    prm.add_parameter("PartitionV",
                      partition_v,
                      "Number of partitions of the v-triangulation.");
    prm.add_parameter("UseVirtualTopology",
                      use_virtual_topology,
                      "Renumber processes along a blocked z-curve.");
    prm.add_parameter(
      "UseSharedMemory",
      use_shared_memory,
      "Explicitly use MPI-3.0 shared-memory features during ghost-values-update and compression steps.");
...
print_parameters(const bool print_details, const std::string &label)
{
  ...
  hyperdeal::advection::Parameters<Number> prm_advection;
  prm_advection.add_parameters(prm);

  hyperdeal::advection::cases::Parameters prm_cases(label);
  prm_cases.add_parameters(prm);

  const auto initializer =
    hyperdeal::advection::cases::get<DIM_X, DIM_V, DEGREE, Number>(label);
