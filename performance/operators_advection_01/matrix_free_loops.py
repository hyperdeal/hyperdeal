...
def run_instance(dim_x, dim_v, degree, n, c, s, sm, buffering, ecl, overlapping):
    ...
    with open("./node%s/inputs%s_%s_%s_%s_%s.json" % (str(n).zfill(4), str(c).zfill(2), str(ecl), str(sm), str(buffering), str(overlapping)), 'w') as f:
