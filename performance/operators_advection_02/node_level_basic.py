...
do
    ...
    if [ -d \"k$K\" ]; then
        array=($(ls k$K/*.json))
        likwid-mpirun -np 48 -f -g CACHES   -m -O ./operators_advection_02 \"${{array[@]}}\" | tee caches$K.o | tee -a caches.out
        likwid-mpirun -np 48 -f -g FLOPS_DP -m -O ./operators_advection_02 \"${{array[@]}}\" | tee flops$K.o  | tee -a flops.out
    fi
...
def run_instance(version, type, v, k, dim_x, dim_v, s, do_collocation, do_ecl, detail):
    ...
    datastore["General"]["Mapping"]    = version
...
def main():
    ...
    parser.add_argument('--full', help='foo help', action="store_true")
    parser.add_argument('--cartesian', help='foo help', action="store_true")
    args = parser.parse_args()
    
    # make sure that only one configuaration has been selected
    if((int(args.full) + int(args.cartesian)) != 1):
    ...
    if(args.full):
        folder_name    = "node_level_basic_full"
        version        = "full"
    elif(args.cartesian):
        folder_name    = "node_level_basic_cartesian"
        version        = "cartesian"
    else:
        sys.exit("No configuration has been selected!") 
    ...
    shutil.copy("../../operators_advection_02", ".")
    ...
    for vlen in vlens:
        for degree in degrees:
            ...
            for dim in range(2,7):
                ...
                # create json files
                run_instance(version, type, vlen, degree, dim_x, dim_v, util.compute_grid_pair(dim_x, dim_v, s), do_collocation, do_ecl, detail)
